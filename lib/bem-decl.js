...
BemDecl.prototype.parsed = function() {
    return _.chain(this.stash)
                ...
                .filter(
                    // not allowed blocks
                    function (item) {
                        return this.allowed.length  // non empty allowed blocks list
                                ? _.indexOf(this.allowed, item.block) !== -1
                                    ? true
                                    : false
                                : true
                                ;
...
BemDecl.prototype.handleElemMod = function (block, entity) {
    ...
    if (!hasElemS && !hasElemP) {
        ...
        var posAsObject = _.findIndex(block.elems, { elem: entity.elem }),
            posAsString = _.indexOf(block.elems, entity.elem);

        if (posAsObject !== -1 && posAsString === -1) {
            // "elems" as array of objects
            // found current elem in "elems"
            var me = block.elems[posAsObject],
            ...
            if (hasModS && !hasModP) {
                ...
                // exists "mods" (only)
                var current = _.findIndex(block.elems[posAsObject].mods, { mod: entity.modName });

                if (current !== -1) {
                    // the current mod found in "mods"
                    var exMod = block.elems[posAsObject].mods[current];
                    exMod = this.transformVal(exMod, entity);
                }
                else {
                    // the current mod not found in "mods"
                    // add new mod object to "mods"
                    block.elems[posAsObject].mods.push({ mod: entity.modName, val: entity.modVal });
                }
            }
        }
            ...
            var strings = block.elems;
            block.elems = [];

            _.each(strings, function (elem) {
                var el = { elem: elem };
